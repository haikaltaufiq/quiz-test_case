    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-gray-200">
            <a href="{{ route('participant.dashboard') }}"
                class="text-xs text-gray-500 hover:text-gray-900 flex items-center">
        ...
        @if ($hasUngraded)
            <div
                class="bg-yellow-50 border border-yellow-200 rounded-md p-3.5 text-xs text-yellow-800 flex items-start space-x-2">
                <svg class="h-4 w-4 text-yellow-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor"
                ...
                <div>
                    ...
                    <p class="mt-0.5">Your score currently reflects only multiple-choice questions. An instructor needs
                        to manually review and grade your essay answers. Once graded, your final score will be updated
                        automatically.</p>
                </div>
            </div>
        @endif
            ...
            @foreach ($attempt->answers as $index => $answer)
                <div class="bg-white border border-gray-200 rounded-md p-4 space-y-3 relative pr-52">
                    ...
                    <div class="border-t border-gray-50 pt-3 pl-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Submitted -->
                        <div class="space-y-1">
                            <span class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider block">Your
                                Submission</span>
                            @if ($answer->question->type === 'multiple_choice')
                            ...
                            @else
                                <div
                                    class="p-2 border border-gray-150 rounded bg-gray-50 text-xs text-gray-850 whitespace-pre-line">
                        ...
                        <div class="space-y-1">
                            ...
                            @else
                                <span
                                    class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider block">Grade
                                    Status</span>
                                @if (is_null($answer->is_correct))
                                    <div
                                        class="p-2 border border-yellow-100 rounded bg-yellow-50 text-yellow-800 text-xs">
                                ...
                                @else
                                    <div
                                        class="p-2 border border-red-100 rounded bg-red-50 text-red-800 text-xs font-semibold">
