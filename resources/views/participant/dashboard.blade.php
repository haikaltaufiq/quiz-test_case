<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="pb-4 border-b border-gray-200">
            ...
            <p class="text-xs text-gray-500 mt-1">Select an active quiz below or track your attempt performance history.</p>
        </div>

        <!-- Detailed Statistics Grid -->
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1">
                <span class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider block">Completed Quizzes</span>
            ...
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1 border-l-yellow-400 border-l-2">
                <span class="text-[10px] text-yellow-600 font-semibold uppercase tracking-wider block">Pending Grading Reviews</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <!-- Search Input -->
                    <form action="{{ route('participant.dashboard') }}" method="GET" class="flex items-center space-x-2 w-full sm:w-auto">
                        <x-ui.input name="search" type="text" placeholder="Search assessments..." value="{{ $search }}" class="!py-1 !text-xs max-w-[200px]" />
                        <x-ui.button type="submit" variant="secondary" class="!px-2 !py-1 !text-xs">Search</x-ui.button>
                        @if($search)
                            <a href="{{ route('participant.dashboard') }}" class="text-xs text-gray-500 hover:text-gray-900">Clear</a>
                        @endif
                    </form>
                </div>
                ...
                @if($quizzes->isEmpty())
                    <div class="border border-dashed border-gray-200 rounded-md p-8 text-center bg-white">
                        ...
                        <p class="mt-1 text-xs text-gray-400">There are no quizzes configured matching your criteria at the moment.</p>
                    </div>
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($quizzes as $quiz)
                            <div class="bg-white border border-gray-200 rounded-md p-4 flex flex-col justify-between hover:border-gray-300 transition-colors">
                                <div class="space-y-2">
                                    <div class="flex items-start justify-between">
                                        <h3 class="text-sm font-semibold text-gray-900 leading-5">{{ $quiz->title }}</h3>
                                    </div>
                                    <div class="flex flex-wrap gap-1.5 mt-1.5">
                                        <x-ui.badge variant="gray">{{ $quiz->duration_minutes }} mins</x-ui.badge>
                                        <x-ui.badge variant="blue">{{ $quiz->questions_count }} {{ Str::plural('question', $quiz->questions_count) }}</x-ui.badge>
                                        ...
                                        @else
                                            <x-ui.badge variant="yellow">Max {{ $quiz->max_attempts }} Attempts</x-ui.badge>
                                        @endif
                                    </div>
                                    <p class="text-xs text-gray-500 mt-2 line-clamp-3">{{ $quiz->description ?? 'No description provided.' }}</p>
                                </div>
                                    ...
                                    @if($quiz->questions_count > 0)
                                        @if($quiz->active_attempt)
                                            <a href="{{ route('participant.attempts.take', $quiz->active_attempt) }}">
                                                <x-ui.button variant="primary" class="!px-3 !py-1 !text-xs !bg-amber-600 hover:!bg-amber-700 !border-amber-600 text-white animate-pulse">
                                                    Resume Attempt
                                                </x-ui.button>
                                            </a>
                                        @elseif($quiz->is_limit_reached)
                                            <span class="text-[11px] font-semibold text-red-600 bg-red-50 border border-red-200 px-2 py-0.5 rounded">{{ $quiz->limit_message }}</span>
                                        @else
                                            <form action="{{ route('participant.quizzes.start', $quiz) }}" method="POST">
                                                @csrf
                                                <x-ui.button type="submit" variant="primary" class="!px-3 !py-1 !text-xs">
                                                    Start Assessment
                                                </x-ui.button>
                                            </form>
                                        @endif
                            ...
                            <div class="bg-white border border-gray-200 rounded-md p-3.5 space-y-3 hover:border-gray-300 transition-colors">
                                <div class="flex justify-between items-start">
                                    <div class="space-y-0.5">
                                        <span class="text-xs font-semibold text-gray-950 block truncate max-w-[180px]">{{ $attempt->quiz->title }}</span>
                                        <span class="text-[10px] text-gray-400 block">{{ $attempt->created_at->format('M d, Y - H:i') }}</span>
                                    </div>
                                    <div class="text-right">
                                        @if($attempt->completed_at)
                                            ...
                                            @if($hasUngraded)
                                                ...
                                                <span class="text-[10px] text-gray-400 block mt-1">Score: {{ $attempt->score }}% (MC)</span>
                                            @else
                                ...
                                <div class="flex justify-between items-center pt-2.5 border-t border-gray-100">
                                    ...
                                    @if($attempt->completed_at)
                                        <a href="{{ route('participant.attempts.show', $attempt) }}">
                                            <x-ui.button variant="secondary" class="!px-2 !py-0.5 !text-[11px]">View Results</x-ui.button>
                                        </a>
                                    @else
                                        <a href="{{ route('participant.attempts.take', $attempt) }}">
                                            <x-ui.button variant="primary" class="!px-2 !py-0.5 !text-[11px]">Resume</x-ui.button>
                                        </a>
                                    @endif
                                </div>
                            </div>
