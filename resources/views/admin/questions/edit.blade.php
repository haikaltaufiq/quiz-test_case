<x-app-layout>
    <x-slot:title>Edit Question - {{ $quiz->title }}</x-slot:title>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div>
            <h1 class="text-lg font-bold text-gray-900 tracking-tight">Edit Question</h1>
            <p class="text-xs text-gray-500 mt-1">Modify details for this question in "{{ $quiz->title }}".</p>
        </div>

        <div class="bg-white border border-gray-200 rounded-md p-6" x-data="{ type: '{{ old('type', $question->type) }}' }">
            <form action="{{ route('admin.questions.update', [$quiz, $question]) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="question_text" name="question_text" rows="3" required class="block w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-md px-3 py-2 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900" placeholder="Type the question content here...">{{ old('question_text', $question->question_text) }}</textarea>
                </div>
                ...
                <div x-show="type === 'multiple_choice'" class="space-y-4 border-l-2 border-gray-200 pl-4 py-1" x-transition>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-ui.input id="option_a" name="options[a]" type="text" value="{{ old('options.a', $question->options['a'] ?? '') }}" placeholder="Enter option A value" ::required="type === 'multiple_choice'" />
                        </div>
                        <div>
                            <x-ui.label for="option_b" value="Option B" />
                            <x-ui.input id="option_b" name="options[b]" type="text" value="{{ old('options.b', $question->options['b'] ?? '') }}" placeholder="Enter option B value" ::required="type === 'multiple_choice'" />
                        </div>
                        <div>
                            <x-ui.label for="option_c" value="Option C" />
                            <x-ui.input id="option_c" name="options[c]" type="text" value="{{ old('options.c', $question->options['c'] ?? '') }}" placeholder="Enter option C value" ::required="type === 'multiple_choice'" />
                        </div>
                        <div>
                            <x-ui.label for="option_d" value="Option D" />
                            <x-ui.input id="option_d" name="options[d]" type="text" value="{{ old('options.d', $question->options['d'] ?? '') }}" placeholder="Enter option D value" ::required="type === 'multiple_choice'" />
                        </div>
                    </div>
                ...
                <div x-show="type === 'essay'" class="space-y-3 border-l-2 border-gray-200 pl-4 py-1" x-transition>
                    ...
                    <div>
                        ...
                        <textarea id="correct_answer_essay" name="correct_answer" rows="4" class="block w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-md px-3 py-2 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900" placeholder="Type the keywords or ideal response details to assist manual grading..." ::required="type === 'essay'" :disabled="type !== 'essay'">{{ old('correct_answer', $question->correct_answer) }}</textarea>
                    </div>
                </div>
