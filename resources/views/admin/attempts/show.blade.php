<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div
            ...
            <div>
                <span class="text-xs text-gray-400 font-semibold block uppercase tracking-wider">Participant
                    Attempt</span>
            ...
            <div class="flex items-center space-x-6">
                ...
                <div class="text-left md:text-right text-xs text-gray-500">
                    ...
                    @if ($attempt->completed_at)
                        Finished: {{ $attempt->completed_at->format('M d, H:i') }}
                ...
                @foreach ($attempt->answers as $index => $answer)
                    <div class="bg-white border border-gray-200 rounded-md p-4 space-y-3 relative pr-52">
                        ...
                        <div class="flex items-start">
                            <p class="text-sm font-semibold text-gray-900 whitespace-pre-line">{{ $index + 1 }}.
                                {{ $answer->question->question_text }}</p>
                        </div>
                        ...
                        <div class="border-t border-gray-50 pt-3 pl-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-1">
                                <span
                                    class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider block">Submitted
                                    Answer</span>
                                @if ($answer->question->type === 'multiple_choice')
                                    <div class="p-2 border border-gray-150 rounded bg-gray-50 text-xs">
                                        @if ($answer->submitted_answer)
                                            <span
                                                class="uppercase font-bold mr-1.5">{{ $answer->submitted_answer }}:</span>
                                ...
                                @else
                                    <div
                                        class="p-2 border border-gray-150 rounded bg-gray-50 text-xs text-gray-800 whitespace-pre-line">
                            ...
                            <div class="space-y-1.5">
                                @if ($answer->question->type === 'multiple_choice')
                                    <span
                                        class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider block">Correct
                                        Key</span>
                                    <div
                                        class="p-2 border border-emerald-100 rounded bg-emerald-50 text-emerald-800 text-xs font-semibold">
                                        <span
                                            class="uppercase font-bold mr-1.5">{{ $answer->question->correct_answer }}:</span>
                                        <span>{{ $answer->question->options[$answer->question->correct_answer] ?? '' }}</span>
                                    </div>
                                @else
                                    <span
                                        class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider block">Ideal
                                        Answer Guide</span>
                                    <div
                                        class="p-2 border border-gray-150 rounded bg-white text-xs text-gray-600 whitespace-pre-line mb-3">
                                    ...
                                    @if ($attempt->completed_at)
                                        ...
                                        <div
                                            class="flex items-center space-x-4 p-2 bg-gray-50 border border-gray-100 rounded-md">
                                            <span class="text-xs font-medium text-gray-700">Mark as:</span>
                                            <div class="flex items-center space-x-3">
                                                <label
                                                    class="inline-flex items-center cursor-pointer text-xs text-gray-700">
                                                    <input type="radio" name="grades[{{ $answer->id }}]"
                                                        value="correct"
                                                        {{ $answer->is_correct === true ? 'checked' : '' }}
                                                        class="h-3.5 w-3.5 text-emerald-600 focus:ring-emerald-500 border-gray-300">
                                                ...
                                                <label
                                                    ...
                                                    <input type="radio" name="grades[{{ $answer->id }}]"
                                                        ...
                                                        class="h-3.5 w-3.5 text-red-600 focus:ring-red-500 border-gray-300">
    ...
    @if ($attempt->completed_at && $attempt->quiz->questions->where('type', 'essay')->isNotEmpty())
