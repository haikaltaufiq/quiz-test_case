<x-app-layout>
    <x-slot:title>Edit Quiz - Haikal Test-Case</x-slot:title>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div>
            <h1 class="text-lg font-bold text-gray-900 tracking-tight">Edit Quiz Details</h1>
            <p class="text-xs text-gray-500 mt-1">Modify metadata and configuration for "{{ $quiz->title }}".</p>
        </div>

        <div class="bg-white border border-gray-200 rounded-md p-6">
            <form action="{{ route('admin.quizzes.update', $quiz) }}" method="POST" class="space-y-4" x-data="{ attemptType: '{{ old('attempt_limit_type', $quiz->attempt_limit_type) }}' }">
                @csrf
                @method('PUT')

                <div>
                    <x-ui.label for="title" value="Quiz Title" />
                    <x-ui.input id="title" name="title" type="text" required value="{{ old('title', $quiz->title) }}" placeholder="e.g. Web Security Basics" />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-ui.input id="duration_minutes" name="duration_minutes" type="number" min="1" max="480" required value="{{ old('duration_minutes', $quiz->duration_minutes) }}" placeholder="e.g. 30" class="w-full" />
                ...
                <div x-show="attemptType === 'custom'" x-transition class="p-3 bg-gray-50 border border-gray-150 rounded-md">
                    ...
                    <x-ui.input id="max_attempts" name="max_attempts" type="number" min="2" max="100" value="{{ old('max_attempts', $quiz->max_attempts ?? 2) }}" placeholder="e.g. 3" class="max-w-[200px]" />
                ...
                <div class="flex items-center space-x-2.5 pt-1.5">
                    <input type="checkbox" id="is_published" name="is_published" value="1" {{ old('is_published', $quiz->is_published) ? 'checked' : '' }} class="h-4 w-4 text-gray-900 border-gray-200 rounded focus:ring-gray-950 cursor-pointer">
