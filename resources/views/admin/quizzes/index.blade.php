    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-gray-200 gap-4">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Manage quizzes, configure questions, and grade participant answers.</p>
            </div>
            <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1">
                <span class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider block">Total Quizzes</span>
                <span class="text-xl font-bold text-gray-900 block">{{ $stats['total_quizzes'] }}</span>
            </div>
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1">
                <span class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider block">Published Quizzes</span>
                <span class="text-xl font-bold text-gray-900 block">{{ $stats['published_quizzes'] }}</span>
            </div>
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1">
                <span class="text-[10px] text-gray-400 font-semibold uppercase tracking-wider block">Total Attempts</span>
            ...
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1 border-l-yellow-400 border-l-2">
                <span class="text-[10px] text-yellow-600 font-semibold uppercase tracking-wider block">Needs Review</span>
            ...
            <div class="bg-white border border-gray-200 rounded-md p-4 space-y-1 border-l-emerald-400 border-l-2">
                <span class="text-[10px] text-emerald-600 font-semibold uppercase tracking-wider block">Average Score</span>
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <form action="{{ route('admin.dashboard') }}" method="GET" class="flex items-center space-x-2 w-full sm:w-auto">
                        ...
                        @endif
                        <x-ui.input name="search_quizzes" type="text" placeholder="Search quizzes..." value="{{ $searchQuizzes }}" class="!py-1 !text-xs max-w-[200px]" />
                        <x-ui.button type="submit" variant="secondary" class="!px-2 !py-1 !text-xs">Search</x-ui.button>
                        @if($searchQuizzes)
                            <a href="{{ route('admin.dashboard', ['search_attempts' => $searchAttempts]) }}" class="text-xs text-gray-500 hover:text-gray-900">Clear</a>
                        @endif
                    </form>
                </div>
                ...
                @if($quizzes->isEmpty())
                    <div class="border border-dashed border-gray-200 rounded-md p-8 text-center bg-white">
                        ...
                        <p class="mt-1 text-xs text-gray-400">Try adjusting your keywords or create a new online assessment quiz.</p>
                    </div>
                @else
                    <div class="space-y-3">
                        @foreach($quizzes as $quiz)
                            <div class="bg-white border border-gray-200 rounded-md p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center hover:border-gray-300 transition-colors gap-4">
                                <div class="space-y-1">
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        <a href="{{ route('admin.quizzes.show', $quiz) }}" class="text-sm font-semibold text-gray-900 hover:text-gray-700 hover:underline">
                                            {{ $quiz->title }}
                                        </a>
                                    ...
                                    <p class="text-xs text-gray-500 line-clamp-2 max-w-xl">{{ $quiz->description ?? 'No description provided.' }}</p>
                                </div>
                                <div class="flex items-center space-x-2 w-full sm:w-auto justify-end">
                                    <form action="{{ route('admin.quizzes.toggle-publish', $quiz) }}" method="POST">
                                        ...
                                        @method('PATCH')
                                        @if($quiz->is_published)
                                            <x-ui.button type="submit" variant="secondary" class="!px-2.5 !py-1 !text-xs !bg-amber-50 hover:!bg-amber-100 !text-amber-800 !border-amber-200">
                                                Revert to Draft
                                            </x-ui.button>
                                        @else
                                            <x-ui.button type="submit" variant="primary" class="!px-2.5 !py-1 !text-xs !bg-emerald-600 hover:!bg-emerald-700 !border-emerald-600 text-white">
                                                Publish Draft
                                            </x-ui.button>
                                        @endif
                                    </form>
                                    <a href="{{ route('admin.quizzes.show', $quiz) }}">
                                        <x-ui.button variant="secondary" class="!px-2.5 !py-1 !text-xs">View & Edit</x-ui.button>
                                    </a>
                                    <form action="{{ route('admin.quizzes.destroy', $quiz) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this quiz? All associated questions and student attempts will be soft-deleted.')">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <form action="{{ route('admin.dashboard') }}" method="GET" class="flex items-center space-x-2 w-full sm:w-auto">
                        ...
                        @endif
                        <x-ui.input name="search_attempts" type="text" placeholder="Search candidate..." value="{{ $searchAttempts }}" class="!py-1 !text-xs max-w-[150px]" />
                        <x-ui.button type="submit" variant="secondary" class="!px-2 !py-1 !text-xs">Search</x-ui.button>
                        @if($searchAttempts)
                            <a href="{{ route('admin.dashboard', ['search_quizzes' => $searchQuizzes]) }}" class="text-xs text-gray-500 hover:text-gray-900">Clear</a>
                        @endif
                    </form>
                </div>
                ...
                @else
                    <div class="space-y-3">
                        @foreach($attempts as $attempt)
                            <div class="bg-white border border-gray-200 rounded-md p-3.5 space-y-3 hover:border-gray-300 transition-colors">
                                <div class="flex items-start justify-between">
                                    <div class="space-y-0.5">
                                        <span class="text-xs font-semibold text-gray-950 block">{{ $attempt->user->name }}</span>
                                        <span class="text-xs text-gray-500 block truncate max-w-[180px]">{{ $attempt->quiz->title }}</span>
                                    </div>
                                    <div class="text-right">
                                        @if($attempt->completed_at)
                                            @php
                                                $hasUngradedEssays = $attempt->answers()
                                        ...
                                        @endif
                                        <span class="text-[10px] text-gray-400 block mt-1">{{ $attempt->created_at->diffForHumans() }}</span>
                                    </div>
                                </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-600">
                        @forelse($activityLogs as $log)
                            <tr class="hover:bg-gray-50/50">
                                <td class="px-4 py-2.5 whitespace-nowrap text-gray-400">{{ $log->created_at->format('M d, H:i:s') }}</td>
                                <td class="px-4 py-2.5 whitespace-nowrap font-medium text-gray-950">{{ $log->user ? $log->user->name : 'System' }}</td>
                                <td class="px-4 py-2.5 whitespace-nowrap">
                                    @php
                                        $badgeVar = 'gray';
                                        if (Str::contains($log->action, 'created')) $badgeVar = 'blue';
                                        if (Str::contains($log->action, 'published')) $badgeVar = 'green';
                                        if (Str::contains($log->action, 'submitted')) $badgeVar = 'green';
                                        if (Str::contains($log->action, 'deleted')) $badgeVar = 'red';
                                        if (Str::contains($log->action, 'graded')) $badgeVar = 'yellow';
                                    @endphp
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-6 text-center text-gray-400 italic">No activities logged yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
